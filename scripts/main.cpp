...
int main(int argc, char** argv)
{
  ...
  for(int loop = 0; loop < 1000; loop++){
	...
	for(int i = 0; i < input_data.size(); i++){
	  ...
	  else if(ndt_description.at(ref_id).size < 3){
		continue;
	  }
	...
	if( estimateInvMat3d(Hesse, inv_Hesse) ){
	  ...
	  //paramのupdate
	  opt_param[0] += (delta_p[0] * step);
	  opt_param[1] += (delta_p[1] * step);
	  opt_param[2] += (delta_p[2] * step);
	}
	else{
	  continue;
	}
	std::cerr << "update_param[" << loop << "](" << opt_param[0] << ", " << opt_param[1] << ", " << opt_param[2] << ")" << " score: " << score <<std::endl;
  }
  return 0;
}
